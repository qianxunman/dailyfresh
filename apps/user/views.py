...
class LoginView(View):
    ...
    def post(self, request):
        ...
        if not all([username, password]):
            return render(request, 'login.html', {'errmsg': 'required data not provide'})
        ...
        if user is not None:
            if user.is_active:
                ...
                return render(request, 'login.html', {'errmsg': 'the user is not active'})
        else:
            return render(request, 'login.html', {'errmsg': 'incorrect password or username'})
...
class UserInfoView(LoginRequiredMixin, View):
    ...
    def get(self, request):
        ...
        # 获取用户的个人信息
        user = request.user
        address = Address.objects.get_defalut_address(user)
